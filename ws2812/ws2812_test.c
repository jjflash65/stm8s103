...
#define gpio_pin      4                         // Portpin an dem die LED angeschlossen ist
                                                // hier dann PD4
...
void setrgbfromega(uint8_t eganr)
{
  rgbcolor.g= egapalette[(eganr*3)+1];        // gruen
  rgbcolor.r= egapalette[(eganr*3)];          // rot
  rgbcolor.b= egapalette[(eganr*3)+2];        // blau
}
...
/* ----------------------------------------------------------
      ...
      setzt den Farbwert der in der globalen Variable
      ...
      Parameter:
                    *ptr : Zeiger auf ein Array, das die
                      ...
                      nr : LED-Nummer im Array, welche die
                           ...
                           repraesentiert
   ---------------------------------------------------------- */
void ws_setrgbcol(uint8_t *ptr, uint16_t nr)
{
  ptr+= (nr*3);
  *ptr= rgbcolor.g;
  ptr++;
...
/* ----------------------------------------------------------
      ...
      blendet eine Anzahl anz mit dem Farbwert, der in der
      globalen Variable

                             colvalue rgbcolor

      angegeben ist LED Reihe links schiebend auf.

      Verzoegerungszeit dtime bestimmt die Dauer eines
      Einzelschrittes beim Aufbau
   ---------------------------------------------------------- */
void ws_blendup_left(uint8_t *ptr, uint8_t anz, int dtime)
{
  int8_t i;
...
/* ----------------------------------------------------------
      ...
      angegeben ist LED Reihe rechts schiebend auf.

      Verzoegerungszeit dtime bestimmt die Dauer eines
      Einzelschrittes beim Aufbau
   ---------------------------------------------------------- */
void ws_blendup_right(uint8_t *ptr, uint8_t anz, int dtime)
{
  int8_t i;
...
int main(void)
{
  ...
  while(1)
  {
    ...
    {
      setrgbcolor(0,2,0);
      ws_blendup_left(&ledbuffer[0], ledanz, 35);
      setrgbcolor(2,0,0);
      ws_blendup_left(&ledbuffer[0], ledanz, 35);
      setrgbcolor(0,0,2);
      ws_blendup_left(&ledbuffer[0], ledanz, 35);

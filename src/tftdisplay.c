/* -------------------------------------------------------
     ...
     Softwaremodul fuer Color TFT-Displays
...
/* ----------------------------------------------------------
   ...
   ----------------------------------------------------------*/
void putstring(char *c)                              // Uebergabe eines Zeigers (auf einen String)
{
  while (*c)
  {
    lcd_putchar(*c++);
...
#if (fastxline_enable == 1 || fillrect_enable == 1)
...
#if (fillellipse_circle_enable == 1)
